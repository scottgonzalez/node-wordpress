...
extend( Client.prototype, {
	createCategory: function( name, slug, parent, fn ) {
		if ( typeof parent === "function" ) {
			fn = parent;
			parent = 0;
		}

		this.createTerm( name, slug, function( error, termId ) {
			if ( error ) {
				return fn( error );
			}

			this.createTermTaxonomy( termId, "category", parent, function( error, termTaxonomyId ) {
				if ( error ) {
					return fn( error );
				}

				fn( null, termTaxonomyId );
			}.bind( this ) );
		}.bind( this ) );
	},

	addCategory: function( postId, slug, fn ) {
		this.db.query( "SELECT `term_taxonomy_id` FROM `" + this.tables.term_taxonomy + "` " +
			"WHERE `taxonomy` = 'category' AND `term_id` = (" +
				"SELECT `term_id` FROM `" + this.tables.terms + "` " +
				"WHERE `slug` = ?)",
			[ slug ], function( error, rows ) {
				if ( error ) {
					return fn( error );
				}

				if ( !rows.length ) {
					return fn( new Error( "Invalid category" ) );
				}
