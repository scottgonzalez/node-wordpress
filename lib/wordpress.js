...
function convertObjectFromCamel(filter){
    var newParam, value;
    ...
    for( param in filter){
        ...
        value = filter[param];
        newParam = param.replace(/([A-Z])/g, function(str, g){return '_'+g.toLowerCase()});
        if( newParam  != param){
            filter[newParam] = filter[param];
            delete filter[param];
        }
    }
    return filter;
 ...
 */
extend( Client.prototype, {
    getMediaItem: function(fn){
        this.authenticatedCall( "wp.getMediaItem", function( error, attachment ) {
            if ( error ) {
                return fn( error );
            }

            fn( null, fieldMap.from( attachment, "attachment" ) );
        });
    },
    ...
    getMediaLibrary: function( filter, fn ){
        ...
        convertObjectFromCamel(filter);

        this.authenticatedCall( "wp.getMediaLibrary", filter, function( error, attachments ) {
            if ( error ) {
                return fn( error );
            }

            fn( null, attachments.map(function( attachment ) {
                return fieldMap.from( attachment, "attachment" );
            }));
        });
    },
    ...
    /**
     *
     * @param data {fileName, type, bits, overwrite}
     * @param fn
     */
    uploadFile: function( data, fn ){
        if( !data.type ){
            var mime = require('mime');
            data.type = mime.lookup( data.name );
        }

        convertObjectFromCamel( data );

        this.authenticatedCall( "wp.uploadFile", data, function( error, file ) {
            if ( error ) {
                return fn( error );
            }

            fn( null, fieldMap.from( file, "file" ) );
        });
