...
extend( Client.prototype, {
	getPost: function( id, fn ) {
		this.call( "wp.getPost", id, function( error, post ) {
			if ( error ) {
				return fn( error );
			}
	...
	getPosts: function( filter, fn ) {
		...
		this.call( "wp.getPosts", filter, function( error, posts ) {
			if ( error ) {
				return fn( error );
			}
	...
	newPost: function( data, fn ) {
		this.call( "wp.newPost", fields.to( data, "post" ), fn );
	},
	...
	editPost: function( id, data, fn ) {
		this.call( "wp.editPost", id, fields.to( data, "post" ), fn );
	},
	...
	getPostType: function( name, fn ) {
		this.call( "wp.getPostType", name, function( error, postType ) {
			...
			fn( null, fields.from( postType, "postType" ) );
		});
	},
	...
	getPostTypes: function( fn ) {
		this.call( "wp.getPostTypes", function( error, postTypes ) {
			...
			Object.keys( postTypes ).forEach(function( postType ) {
				postTypes[ postType ] = fields.from( postTypes[ postType ], "postType" );
			});
			fn( null, postTypes );
		});
	}
...
extend( Client.prototype, {
	...
	newTerm: function( data, fn ) {
		this.call( "wp.newTerm", fields.to( data, "term" ), fn );
	},

	editTerm: function( id, data, fn ) {
		this.call( "wp.editTerm", id, fields.to( data, "term" ), fn );
	},
